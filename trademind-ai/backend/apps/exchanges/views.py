...
from core.permissions import IsOwnerOrAdmin, IsTenantMember
...
from .serializers import (ExchangeAccountCreateSerializer,
                          ExchangeAccountSerializer,
                          ExchangeAccountUpdateSerializer, ExchangeSerializer)
    ...
    def get(self, request: Request, pk: str):
        try:
            ...
            return error_response(
                "RESOURCE_NOT_FOUND", "Exchange not found.", status_code=404
            )
...
class ExchangeAccountListCreateView(APIView):
    ...
    def post(self, request: Request):
        ...
        logger.info(
            "Exchange account connected: user=%s exchange=%s",
            request.user.id,
            account.exchange_id,
        )
...
class ExchangeAccountDetailView(APIView):
    ...
    def _get_account(self, pk: str, user) -> ExchangeAccount | None:
        try:
            return ExchangeAccount.objects.get(
    ...
    def get(self, request: Request, pk: str):
        ...
        if not account:
            return error_response(
                "RESOURCE_NOT_FOUND", "Exchange account not found.", status_code=404
            )
    ...
    def patch(self, request: Request, pk: str):
        ...
        serializer = ExchangeAccountUpdateSerializer(
            account, data=request.data, partial=True
        )
        if not serializer.is_valid():
            return error_response(
                "VALIDATION_ERROR", "Update failed.", details=serializer.errors
            )
    ...
    def delete(self, request: Request, pk: str):
        ...
        logger.info(
            "Exchange account disconnected: user=%s account=%s", request.user.id, pk
        )
...
class ExchangeAccountTestView(APIView):
    ...
    def post(self, request: Request, pk: str):
        ...
        return success_response(
            message="Connection test initiated. Check status in a moment."
        )
...
class ExchangeAccountSyncView(APIView):
    ...
    def post(self, request: Request, pk: str):
        ...
        return success_response(
            message="Sync initiated. Balance and positions will update shortly."
        )
...
class ExchangeAccountBalancesView(APIView):
    ...
    def get(self, request: Request, pk: str):
        ...
        try:
            ...
            assets = PortfolioAsset.objects.filter(portfolio=portfolio).order_by(
                "-value_usdt"
            )
            assets_data = [
                {
