...
class ExchangeSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            ...
            "supports_futures", "supports_spot", "supports_websocket",
            "sandbox_available", "phase",
        ]
...
class ExchangeAccountCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            "exchange", "label", "api_key", "api_secret", "api_passphrase",
            "is_testnet",
        ]
    ...
    def create(self, validated_data: dict) -> ExchangeAccount:
        ...
        return ExchangeAccount.objects.create(
            **validated_data,
            ...
            api_passphrase_encrypted=encrypt_secret(passphrase, enc_key) if passphrase else "",
        )
...
class ExchangeAccountSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ExchangeAccount
        fields = [
            "id", "exchange", "exchange_name", "exchange_slug", "exchange_logo",
            "label", "api_key_masked", "is_active", "is_testnet",
            "connection_status", "last_sync_at", "balance_synced_at",
            "created_at",
        ]
    ...
    def get_api_key_masked(self, obj: ExchangeAccount) -> str:
        ...
        return mask_secret(obj.api_key_encrypted[:12] if obj.api_key_encrypted else "", 4)
...
class ExchangeLogSerializer(serializers.ModelSerializer):
    class Meta:
        model  = ExchangeLog
        fields = ["id", "action", "status_code", "latency_ms", "error_message", "created_at"]
